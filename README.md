...
### 🔍 Диагностика и проверка на сервере:
- **Проверка времени и часового пояса сервера** (должен быть `Time zone: UTC`):
  ```bash
  timedatectl
  ```
- **Просмотр задания cron**:
  ```bash
  crontab -l
  ```
- **Просмотр логов работы**:
  ```bash
  cat /var/log/chan_digest.log
  # или онлайн-мониторинг:
  tail -f /var/log/chan_digest.log
  ```
- **Ручной запуск** (если нужно опубликовать дайджест прямо сейчас):
  ```bash
